...
## Lists
<!-- this repeats almost verbatim the content from the earlier lesson. Keep?-->
...
Components are indexed by double brackets `[[`. Single brackets will return a list *containing* only the indexed components..
...
1. Write a function called `ar1()` taking a single argument `phi` that simulates 20 values from an AR(1) process. An AR(1) is a series where the the current value of the series, `yt`, is given by `yt = (phi * yt-1) + e`, where `phi` is a constant and `e` is white noise. Assume that`yt = 0` at `t = 0` and `rnorm()` can be used for `e`. 

```{r}
ar1 <- function(phi = 0.5) {
    # simulates AR(1)
    start <- 0
    out <- c(start, numeric(length = 20))
    for (i in seq(2, 20, by = 1)) {
        out[i] <- (phi * out[i-1]) + rnorm(1)
    }
    out[-1]
}
```
