...
#Here we will use R's ifelse function, in which we provide the test phrase, the outcome if the result of the test is 'TRUE', and the outcome if the result is 'FALSE'. We will also assign the results to the Color column, using '<-'
...
carSpeeds <- read.csv(file='carSpeeds-01.csv', stringsAsFactors=FALSE)
...
```{r as.is1}
carSpeeds <- read.csv(file='carSpeeds-01.csv', as.is = 1) 
...
```{r Challenge}
carSpeeds <- read.csv(file='carSpeeds-01.csv')
#Replace 'Blue' with 'Green' in cars$Color without re-importing the data set
```
...
```{r strip.whiteFALSE}
#We use the tapply() function, in which we provide the variable we want summarized, the grouping variable, and the type of summary we want
 
tapply(carSpeeds$Speed, carSpeeds$Color, mean)
```
...
```{r strip.whiteTRUE}
carSpeeds<-read.csv(file='carSpeeds-01.csv',stringsAsFactors=FALSE,strip.white=TRUE,sep=',')
...
write.csv(carSpeeds, file='CarSpeeds_Cleaned.csv')
```

Now let's open the .csv and see what it looks like:

<img src="../novice/r/figure/CSV_WithRowNums.png" alt="csv written without row.names argument" />

Notice that the file has header names, because the data had headers within R. But why are there numbers in the first column? 
...
```{r row.namesFALSE}
write.csv(carSpeeds, file='CarSpeeds_Cleaned.csv', row.names=FALSE)
```

<img src="../novice/r/figure/CSV_WithoutRowNums.png" alt="csv written with row.names argument" />
...
write.csv(carSpeeds, file='CarSpeeds_Cleaned.csv', row.names=FALSE, na= '-9999')
```

<img src="../novice/r/figure/CSV_WithoutSpecialNA.png" alt="csv written with -9999 as NA" />
