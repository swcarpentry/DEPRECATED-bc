...
When we call `fahr_to_celsius()`, R doesn't create the object `temp` right away. Instead, it creates a new environment, *local* to the function, to keep track of the objects involved in the `fahr_to_kelvin()` call. Initially, the environment only holds the object `temp`:

Your function has its own environment where variables local to that function are stored (`temp` in this case). Since R is lexically scoped that means that the function's environment has a parent environment which is defined by where the function is called, the global environment in this case (if it were called inside of another function then the parent environment would be the env for that function). <!-- need to think about this; think confusing definition with calling? -->

When we call `fahr_to_kelvin()` inside `fahr_to_celsius()`, R creates another local environment to hold the objects involved in the evaluation of `fahr_to_kelvin()`

When you call `fahr_to_celsius()` it calls `fahr_to_kelvin()` and then `fahr_to_kelvin()` tries to find the variable `temp`, it first looks in its own environment, but if it does not find it there, then looks in its parent environment which is the global env. 

It does this because there are now two objects in play called `temp`: the argument to `fahr_to_celsius()`, and argument to `fahr_to_kelvin()`. 
Having two variables with the same name in the same part of the program would be ambiguous, so R (and every other modern programming language) creates a new environment for each function call to keep that function's objects separated from those in use by other functions.

When the call to `fahr_to_kelvin()` returns a value, R throws away `fahr_to_kelvin()`'s stack frame and creates a new object in the stack frame for `fahr_to_celsius()` to hold the temperature in Kelvin:
