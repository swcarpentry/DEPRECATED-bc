...
```{r}
fahr_to_kelvin(32)
cat('boiling point of water:', fahr_to_kelvin(212))
```
...
cat('absolute zero in Celsius:', kelvin_to_celsius(0))
```
...
cat('freezing point of water in Celsius:', fahr_to_celsius(32.0))
```

Note that the function can return more than one value, but the values have to be grouped in a list or a vector. For example, suppose we want to write a function that converts Fahrenheit to both Kelvin and Celsius, we could write it this way:

```{r}
fahr_to_kelvin_and_celsius <- function(temp) {
  temp_k <- fahr_to_kelvin(temp)
  temp_c <- fahr_to_celsius(temp)
  c(temp_k, temp_c)  # create a vector of values
}
temps <- fahr_to_kelvin_and_celsius(300)
cat("temperature in Kenvin:", temps[1], "\ntemperature in Celsius:", temps[2])
```
...
Functions in R can be anonymous meaning they do not have a name, or technically they are not assigned to a variable. Anonymous functions are usually small (one-liner).

```{r}
function(n) n^2
```

The above function is an example of an anonymous function. You cannot call an anonymous function directly, so it is usually called by another function. For example, in the command below, `sapply` passes each element in `num` as an argument of the anonymous function, which returns a square of a given number.

```{r}
num <- 1:10
squares <- sapply(num, function(n) n^2)

squares
```

Note that {} are omitted in this example because the function only has a single command. Anonymous function can be handy when it is short, however, it makes code unwieldy when it is too long.
...
```{r}
cat('final value of temp after all function calls:', temp)
```
...
numbers  <-  1:20
cat("span of numbers: ", span(numbers))
...
```{r}
cat('original min, mean, and max are:', min(datamatrix), ',', mean(datamatrix), ',', max(datamatrix))
centered <- center(datamatrix, 0)

cat('original min, mean, and max are:', min(centered), ',', mean(centered), ',', max(centered))
```	
...
```{r}
cat('std dev before and after:', sd(datamatrix), sd(centered))
```

Those values look the same, but we probably wouldn't notice if they were different in the sixth decimal place. Let's do this instead:

```{r}
cat('difference in standard deviations before and after:', sd(datamatrix) - sd(centered))
```
...
cat('no parameters:', display())
cat('one parameter:', display(55))
cat('two parameters:', display(55, 66))
cat('three parameters:', display (55, 66, 77))
```

As this example shows, parameters are matched up from left to right, and any that haven't been given a value explicitly get their default value. We can override this behavior by naming the value as we pass it in:

```{r}
cat('only setting the value of c', display(c=77))
```
