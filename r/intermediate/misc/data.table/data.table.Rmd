...
```{r fread}
# First we make a large data frame (10,000 rows long; 1.78 GB) and write it to a csv file, which we'll read in in the next step
...
If you did this with a larger dataset, say 10^8 rows long (ca. 1.8 GB), then the speed up becomes really meaningful (almost 60x faster):
```
> system.time( read.csv("test.csv") )
   user  system elapsed 
512.116   7.311 521.549 
> system.time( fread("test.csv") )
   user  system elapsed 
  7.002   1.092   8.948
```
